...
> [!WARNING]
> 本项目适用于离线机器人对局、由用户自行托管的好友私人房，以及用于机器人对局的私人专用服务器。`BotRandomizer` 仅为游戏识别出的机器人实体分配饰品，不会向真人玩家授予、伪造或改写其 Steam 库存、饰品或个人资料。这一边界旨在遵守 [Valve 针对 CS2 社区服务器及 GSLT 的相关规定](https://help.steampowered.com/zh-cn/faqs/view/07AF-502E-A104-BD4B)。
>
> 本项目不面向、也不支持 Valve 官方匹配、启用 VAC 的公共服务器、[FACEIT](https://support.faceit.com/hc/en-us/articles/360015788779-What-is-deemed-to-be-a-cheat) 或其他第三方公共社区服务器；亦不得将其用于规避反作弊系统、文件校验或其他安全控制。进入上述服务前，请通过 Panel 切换至**联机模式（Online Mode）**，或手动恢复标准游戏文件、移除 `-insecure`，并重启 CS2。
>
> [AGPL-3.0 许可证](../LICENSE)允许依照其条款修改和再分发本项目源码，但并不授予使用第三方服务的准入权，也不构成违反其规则的授权。在适用法律允许的最大范围内，任何人超出上述范围使用或部署本项目、修改本项目以规避安全控制，或以其他方式违反第三方条款的，均应自行承担由此产生的全部风险与责任，包括 GSLT 或服务器处罚、FACEIT 或社区服封禁、VAC 或游戏封禁，以及 Steam 账号限制。维护者和贡献者不对上述后果承担责任。本声明仅用于说明项目边界，不构成法律意见。
